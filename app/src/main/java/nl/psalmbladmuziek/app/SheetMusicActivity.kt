...
class SheetMusicActivity : AppCompatActivity() {
    ...
    private fun readDisplayText(): String = if (isTextOnlyAsset()) {
        ...
        extractLyrics(readAsset(fileName))
    }

    private fun stripInstrumentLabels(musicXml: String): String = musicXml
    ...
    private fun extractLyrics(musicXml: String): String {
        ...
        if (words.isEmpty()) {
            return HymnRepository.verseByFileName(fileName)?.firstLine.orEmpty()
        }
    ...
    companion object {
        ...
        private const val DEFAULT_FILE_NAME = "Psalm116_v1.xml"
